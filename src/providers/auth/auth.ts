...
export class AuthProvider {
  ...
  signInEmail(email,pass) {
    ...
    this.http.post( API_URL+'login',{email: 'parmar.ashesh@tristonsoft.com', password: '123456'},{headers: {'Content-Type': 'application/json','Accept': 'application/json'}}).subscribe(suc => {
      // localStorage.setItem('swaToken',s)      
  ...
  checkLogin() {
    ...
    const guid = 'guid';
  //  if(token !== undefined) {
  //   this.http.post(API_URL+ 'details',{headers: {'public_guid': guid}}).subscribe(user => {
